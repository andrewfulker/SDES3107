...
//  newX = random(0+maxRadius,width-maxRadius);
//  newY = random(0+maxRadius,height-maxRadius);
//  newR = minRadius;

  // create a random position according to mouse position
  //if (mousePressed == false) {
    newX = random(mouseX,mouseX);
    newY = random(mouseY,mouseY);
    newR = 1;
  //} 
  ...
  if (keyPressed == false) {
    stroke(255,200,0);
    strokeWeight(2);
    ellipse(mouseX-mouseRect/2,mouseY-mouseRect/2,mouseRect,mouseRect);
  } 
